    ...
    return new Promise(function (resolve) {
        chrome.storage.local.get("option", function (obj) {
            if (obj.option == undefined) {
                ...
                const result1 = Object.keys(defaultOption).filter(key =>
                    Object.keys(option).indexOf(key) == -1
                );
                ...
                const result2 = Object.keys(option).filter(key =>
                    Object.keys(defaultOption).indexOf(key) == -1
                );
