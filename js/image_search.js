  ...
  for (let i = 0; i < imageFiles.length; i++) {
    ...
    if (obj["url"]) {
      ...
      if ($("table").length) {
        td.empty();
        const link =
          "<a href='" + obj["url"] + "' target='_blank'>Result Link</a>";
        td.append(link);
      }
    }
      ...
      response = await axios
        ...
        .catch((e) => {
          ...
          if ($("table").length == 0 && !converted) {
            obj["notice"] =
              obj["notice"] + "<br>" + getMessage("notice_Yandex2");
          }
        });
      if (obj["error"]) {
        break;
      ...
      if (response) {
        ...
        try {
          ...
          if (response.captcha && response.captcha["captcha-page"]) {
            ...
            const captchaLink =
              "(<a href='" + captchaUrl + "' target='_blank'>CAPTCHA</a>)";
...
async function searchUsingGoogleLens() {
  ...
  for (let i = 0; i < imageFiles.length; i++) {
    const url = `https://images.google.com/?imgidx=${i}`;
    urls.push(url);
    finishedStatuses.push(false);
    window.open(url);
  }

  // 上で開いたGoogle画像検索ページからメッセージが来たら画像を渡す
  chrome.runtime.onMessage.addListener(async (msg) => {
    if (msg.msg === "notification") {
      const imgidx = msg.imgidx;
      const img = imageFiles[imgidx];
      const url = urls[imgidx];

      // sendMessageで画像を送るために文字列に変換
      ...
      chrome.tabs.query({ url: url }, function (tabs) {
        ...
        // 全ての画像を処理したらページを閉じる
        finishedStatuses[imgidx] = true;
        if (finishedStatuses.every((status) => status)) {
          window.close();
        }
      });
    }
...
function blobToBase64(blob) {
  return new Promise((resolve, reject) => {
    ...
    reader.onerror = () => {
      reject(new Error("Blobの読み込み中にエラーが発生しました。"));
    };
